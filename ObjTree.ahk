...
Class _ObjTree {
	__New(ByRef obj, Title:="ObjTree",Options:="+ReadOnly +Resize,GuiShow=w640 h480",ToolTip:=""){
		...
		,Edit:=this.Edit:=this.Gui.AddEdit("y+1 w" (size.1*0.7) " h" (size.2*0.5) (IsAHK_H?" axr aw2/3 ah1/2 ax1/3 ay1/2 -wrap +HScroll":"") " " this.ReadOnly)
		,Edit.Enabled:=false,	fun:=this.EditEdit.Bind(this,TV,LV),Edit.OnEvent("Change",fun)
	...
	EditEdit(TV,LV){
		this.changed:=1
		,this.items[TV.GetSelection(),LV.GetText(row:=LV.GetNext("Selected"))]:=value:=this.Edit.Value
		,LV.Modify(Row,,,value)
	}
	TVAdd(obj,parent:=""){
		for k,v in obj
	...
	TVSelect(LV,TV,item){
		this.LV.Delete(),	this.Edit.Text:="",	this.Edit.Enabled:=false	,text:=TV.GetText(item),	TV.Modify(item,"Select")
	...
	LVEdit(TV,LV,item){
		...
		obj[newkey]:=obj[key],	obj.Delete(key),	TV.Modify(id,,newkey),	TV.Modify(this.items[this.items[id]],"Sort"),	LV.ModifyCol(1,"Sort"),	this.changed:=1
	}
	LVSelect(TV,LV,item){
		if LV.GetCount()<item
			return
		...
		Loop this.ReadOnlyLevel
			if !TV_Item:=TV.GetParent(TV_Item?TV_Item:TV.GetSelection())
				ReadOnly:=1
		if !TV.GetChild(id){
			this.Edit.Enabled:=!ReadOnly,this.Edit.Text:=this.items[id,LV.GetText(item)]
		} else this.Edit.Text:="",this.Edit.Enabled:=false
	}
	LVDoubleClick(TV,LV){
